...
function downloadVapiPdk () {
    # Handle non-incremental build case when there is no vapi-pdk
    if [ -d "vapi-pdk" -a -e "vapi-pdk/buildinfo.txt" ]; then
        current_build_no=`cat vapi-pdk/buildinfo.txt | grep "Build Number" | cut -f2 -d':' | cut -f2 -d' '`
    fi
    build_no=$(curl -s "http://buildapi.eng.vmware.com/ob/build/?product=vapi-core&branch=vapi-main&_format=json&buildstate=succeeded&_order_by=-changeset&_limit=1" | grep -Po '(?<="id":\W)\d+')
...
vapi-pdk/idl-toolkit/bin/vmodl-parser --json-writer build/src/metadata/demo_metadata.json --library vapi-pdk/idl-toolkit/models/com.vmware.vapi.std.vmidl --name demo $vmodldirs
operations=`get_operations build/src/metadata/demo_metadata.json`
